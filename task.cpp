...
int main(int argc, char* argv[]){
    ...
    if (correct_file(argc)) {
        return correct_file(argc);
    }
    ...
    if (((0>hours_b) or (hours_b>23) or (0>min_b) or (min_b>60) or (0>hours_e) or (hours_e>23) or (0>min_e) or (min_e>60)) or (line2[2]!= ':') or (line2[5]!=' ') or (line2[8]!=':')) {
    ...
    while (getline(file, line)) {
        ...
        if (time_all.size()!=0) {
            if ((stoi((time_all.back()).substr(0, 2)) >h) or ((stoi((time_all.back()).substr(0, 2)) == h) and (stoi((time_all.back()).substr(3, 2)) >m)) or !((0<=h) and (h<=23)) or !((0<=m) and (m<=59))) {
